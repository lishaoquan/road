...
<%
	...
	String basePath = request.getScheme() + "://"
			...
			+ context + "/";
%>
<link href="<%=context%>/css/default.css" rel="stylesheet"
	type="text/css" />
<link rel="stylesheet"
...
<script type="text/javascript" src='<%=context%>/js/common/outlook.js'>
...
<body class="easyui-layout" style="overflow-y: hidden" scroll="no">
	...
	<div region="north" split="true" border="false"
		...
		<span style="float: right; padding-right: 20px;" class="head">欢迎
			使用本系统 <a href="#" id="editpass">修改密码</a> <a href="#" id="loginOut">安全退出</a>
		</span> <span style="padding-left: 10px; font-size: 16px;"><img
			src="<%=context%>/images/blocks.gif" width="20" height="20"
		...
		<div id="leftMenu" style="width: 180px; height: auto; padding: 0;">
			<c:forEach items="${menuInfo}" var="menu" varStatus="status">
				<c:if test="${status.index eq 0}">
					<div class="easyui-panel" title="${menu.menuName}"
						collapsible="true" style="width: 180px; height: auto; padding: 0;">
						<ul>
							<c:forEach items="${menu.childrenMenu}" var="secondMenu">
								<li><div>
										<a target="mainFrame" href="${secondMenu.url}">${secondMenu.menuName}</a>
									</div></li>
							</c:forEach>
						</ul>
					</div>
				...
				<c:if test="${status.index gt 0}">
					<div class="easyui-panel" title="${menu.menuName}"
						...
						style="width: 180px; height: auto; padding: 0;">
						<ul>
							<c:forEach items="${menu.childrenMenu}" var="secondMenu">
								<li><div>
										<a target="mainFrame" href="${secondMenu.url}">${secondMenu.menuName}</a>
